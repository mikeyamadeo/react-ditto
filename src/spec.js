...
test.serial(
  'Ditto: spacing api applies styles as expected for Box, Row, and Col',
  async t => {
    render(
      <div>
        <Box
        ...
        <Row id='axis' {...{ mx: true, my: '--', px: '+', py: 0 }}>Ho</Row>
        <Col id='all' {...{ p: '++', m: '-' }}>Ho</Col>
      </div>,
      document.getElementById('root')
    )
...
test.serial(
  'Ditto: flex property is applied to Row & Col but not Box',
  async t => {
    render(
      <div>
        <Box id='box'>hey</Box>
...
test.serial(
  'Row, Col: x & y props apply style properties as expected',
  async t => {
    render(
      <div>
        <Row id='cx' x>Hi</Row>
...
test.serial(
  'Row, Col: space & wrap props apply style properties as expected',
  async t => {
    render(
      <div>
        <Row id='no-wrap' />
...
test.serial('accepts styles via glamorous api', async t => {
  render(
    <div>
      <Col id='col' reverse color='red'>Hi</Col>
    </div>,
    document.getElementById('root')
  )
