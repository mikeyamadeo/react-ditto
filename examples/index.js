...
const colors = { red: '#d34f4f', yellow: '#f4d554', green: '#6deeb6' }

const HealthBar = React.createClass({
  propTypes: { percentHealth: React.PropTypes.number },
  testRef (node) {
    console.log(node)
  },
  render () {
    ...
    return (
      <Box width='100%' height='6px'>
        <Box
          width={`${percentHealth}%`}
          height='100%'
          baseRef={this.testRef}
          backgroundColor={
            percentHealth < 25
              ? colors.red
              : percentHealth < 50 ? colors.yellow : colors.green
          }
        />
      </Box>
    )
...
const Snorlax = () => (
  <img
    id='fox'
    src={require('../assets/snorlax-pokemon-go.png')}
    width='50px'
  />
)
